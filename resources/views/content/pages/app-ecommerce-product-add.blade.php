...
@php
  ...
  if ($edit) {
      ...
      $images = $product->images()->get(); // Get only images
      $imageJson = [];
      foreach ($images as  $image) {
          $index = 0;
          $filename = basename($image->raw_image);
          $existingImages[] = [
              'id'   => $index + 1, // Start from 1 and increment
              'name' => $filename,
              'image_id' => $image->id,
              'url'  => $image->raw_image // Adjust the path as needed
          ];
          $imageJson[] = [
            'id' => $image->id,
...
@section('content')
  <div class="app-ecommerce">
    <form action="{{ $route }}" method="{{ $method }}" id="eCommerceProductAddForm">
      @csrf
      <!-- Add Product -->
      <div
        ...
        <div class="col-12 col-lg-8">
          ...
          <div class="card mb-6">
            ...
            <div class="card-body">
              ...
              <div class="row mb-5 gx-5">
                <div class="col-md-6">
                  <div class="form-floating form-floating-outline">
                    <input type="number" class="form-control" id="productSku" placeholder="00000" name="productSku"
                           value="{{$product->sku ?? ''}}" aria-label="Product SKU">
                ...
                <div class="col-md-6">
                  <div class="form-floating form-floating-outline">
                    <input type="text" class="form-control" id="productBarcode" placeholder="0123-4567"
                           name="productBarcode" value="{{$product->barcode ?? ''}}" aria-label="Product barcode">
              ...
              <!-- Comment -->
              @if ($edit)
                <input type="hidden" id="editDescription" name="description" value="{!! $product->description ?? '' !!}">
              @endif
              <div>
        ...
        <div class="col-12 col-lg-4">
          ...
          <div class="card mb-6">
            ...
            <div class="card-body">
              ...
              <!-- Discounted Price -->
              <div class="form-floating form-floating-outline mb-5">
                <input type="number" class="form-control" id="productDiscountedPrice" placeholder="Discounted Price"
                       name="productDiscountedPrice" value="{{$product->discounted_price ?? ''}}"
                       aria-label="Product discounted price">
                <label for="productDiscountedPrice">Discounted Price</label>
              </div>

              <div class="form-floating form-floating-outline mb-5">
                <input type="number" class="form-control" id="productStocks" placeholder="Stock" name="productStocks"
                       value="{{$product->stock ?? ''}}" aria-label="Product Stock">
            ...
            <div class="card-body">
              ...
              <div class="mb-5 col ecommerce-select2-dropdown">
                <div class="form-floating form-floating-outline w-100 me-4">
                  <select id="productCategory" name="productCategory" class="select2 form-select"
                          data-placeholder="Select Category">
                    @foreach($categories as $category)
                      <option
                        value="{{ $category->id }}" @selected($category->id = $product->category_id ?? 0)>{{ $category->title }}</option>
                    @endforeach
                  </select>
